...
const CompanyApprovalPage: NextPage = () => {
    ...
    const closeStatusModal = (val) => {
        ...
        if(val) getCompanyList(page, pageLimit)
    }
    ...
    const getCompanyList = (page: number, limit = 10) => {
        setShowLoader(true)
        ...
        API.getCompanyList(page, limit, filter, searchQuery)
        .then((res) => {
    ...
    return (
        ...
        {companyModal && <CompanyApprovalModal data={selectedCompany} closeModal={closeStatusModal} />}
        <AdminLayout meta={meta} pageTitle={`Company Approval`}>
            <Container>
            <SpaceBetween>
                <h2>Total Companies Registered({metaData?.pageMeta?.totalItems})</h2>
                <Container_1>
            ...
            <Table
                ...
                { id: 3, title: 'Email', key: 'email', hasNestedObject: false },
                { id: 4, title: 'Registration Number', key: 'registrationNumber', hasNestedObject: false },
                { id: 3, title: 'Whitelist | Amount', key: 'maxBidAmount', hasNestedObject: false,
                render: (key: any) => (<FlexRow><Label className='caps'>{key?.isWhitelisted ? 'Whitelisted' : Number(key?.maxBidAmount) || '-'}</Label></FlexRow>),
                },
                {
                    id: 6,
                ...
                {
                    id: 7, title: 'KYB Status', key: 'kybStatus',
                    render: (key: any) => (<FlexRow><Label className='caps'>{key?.kybStatus || 'NA'}</Label></FlexRow>),
                },
                {
                    id: 8,
                    title: 'Actions',
            ...
            />
            {companyList?.length ? <TablePagination total={metaData?.pageMeta?.totalItems} limit={pageLimit} currentPage={page} setCurrentPage={setPage} /> : null}
            </Container>
        </AdminLayout>
        </>
    )
