...
const SettingFormData: Model.IFormData = {
  ...
  firstName: {
    label: 'First name',
    validationRules: [ValidateRule.requiredRule(' First name'), ValidateRule.minLengthRule(' First name', 1), ValidateRule.maxLengthRule(' First name', 25), ValidateRule.validateNames('FirstName')],
  },
  lastName: {
    label: 'Last name',
    validationRules: [ValidateRule.requiredRule(' Name'), ValidateRule.minLengthRule(' Name', 1), ValidateRule.maxLengthRule(' Name', 25), ValidateRule.validateNames('Name')],
  },
  userName: {
    label: ' User name',
  ...
  walletAddress: {
    ...
    validationRules: [ValidateRule.requiredRuleForWalletId('Wallet address'), ValidateRule.requiredRule('Wallet address')],
  },
  ...
  phoneNo: {
    label: 'Phone No',
    validationRules: [ValidateRule.requiredRule('Phone no '), ValidateRule.validPhoneNumber('Phone No')],
  },
  description: {
    label: 'Description',
    // validationRules: [
      ...
      auth
        ...
        .then(async (res) => {
          if (res?.data?.data) {
            ...
            console.log('>>> error ', res?.error?.error?.message)
            ModalService.open((modalProps) => <ErrorModal desc={res?.error?.error?.message} close={modalProps.close} />)
          }
        })
        .catch((err) => {
          console.log(err)
        ...
        <ProfileDataWrapper>
          ...
          <Container2>
            ...
            <ProfileContainer>
              ...
              <ProfileDetails>
                {['firstName', 'lastName', 'userName', 'walletAddress', 'email', 'phoneNo'].map((eachItem, i) => (
                  <>
                    <FormDiv key={eachItem + i}>
                      ...
                      <input
                        ...
                        // readOnly={edit && eachItem === 'name' ? false : true}
                        id={eachItem + i}
                        name={eachItem}
                ...
                {['description'].map((eachItem, i) => (
                  <>
                    <FormDiv1 key={eachItem + i}>
                      <label htmlFor={eachItem}>{form?.[eachItem]?.label}</label>
                      <textarea id={eachItem + i} name={eachItem} className={edit ? '' : 'cursor-default'} readOnly={edit ? false : true} value={form?.[eachItem]?.value} onChange={onInputChange} maxLength={250} />
