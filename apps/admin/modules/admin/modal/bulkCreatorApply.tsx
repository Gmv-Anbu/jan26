...
const BulkCreatorApply = ({  data, setOpenModal, commonModalPopup }) => {
  ...
  const handleApply = () => {
    if(selectedUser?.length) {
        ...
        API.bulkUpdateUserRole(payload)
        .then(res => {
            if (res.status === 200) {
                commonModalPopup('success', res?.data?.message || 'User role updated successfully')
            } else {
                commonModalPopup('error', res?.data?.message || 'User role updatation failed')
            }
            setOpenModal(false)
        })
        .catch(err => {
            commonModalPopup('error', err?.error?.error?.data?.message || 'User role updatation failed')
            setOpenModal(false)
        })
    } else {
        setShowError(true)
...
export default BulkCreatorApply
