...
const WhiteListUser = ({  data, setOpenModal, commonModalPopup }) => {
    ...
    const handleApply = () => {
        if(selectedUser?.length) {
            ...
            API.whitelistUserBulk(payload)
            .then(res => {
                if (res.status === 200) {
                    commonModalPopup('success', res?.data?.message || 'User whitelisted successfully')
                } else {
    ...
    <ModalBackground>
      <ModalContainer>
        <div>
          ...
          {showError ? <ErrorMsg>Select users before proceeding</ErrorMsg> : null}
          {data?.length 
          ? <div className='mt-10'>
