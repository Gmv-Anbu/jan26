...
const BulkBlockUser = ({  data, setOpenModal, commonModalPopup }) => {
  ...
  const handleApply = () => {
    if(selectedUser?.length) {
        ...
        API.bulkBlockUser(payload)
        .then(res => {
            if (res.status === 200) {
                commonModalPopup('success', res?.data?.message || 'User blocked successfully')
            } else {
                commonModalPopup('error', res?.data?.message || 'Block failed try again later')
            }
            setOpenModal(false)
        })
        .catch(err => {
            console.log('err asd', err)
            commonModalPopup('error', err?.error?.error?.data?.message || 'Block failed try again later')
            setOpenModal(false)
        })
    } else {
        setShowError(true)
