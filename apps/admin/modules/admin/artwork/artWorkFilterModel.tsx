...
const ArtWorkFilterModel = ({ setOpenModal, data, onApplyFilter, onResetFilter }) => {
  ...
  const isValidDate = (data: any) => {
    if(data?.saleType && !data.from && !data.to || data?.type && !data.from && !data.to) {
      return true
    }
    if (!data.from || !data.to) {
      setValidationError('Please select filter to proceed')
      return false
    }
        ...
        <div>
          ...
          <ButtonComponent>
            ...
            <Button2
              disabled={loading}
              onClick={() => {
                if (isValidDate({ from: from, to: to, saleType: saleType, type: type })) {
                  onApplyFilter({ from: from, to: to, saleType: saleType, type: type })
                  setLoading(true)
                  setOpenModal(false)
                }
...
export default ArtWorkFilterModel
