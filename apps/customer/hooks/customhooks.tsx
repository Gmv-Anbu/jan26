...
export function useSortHandler(data: any, sortType: string) {
  // console.log("useSortHandler data changes ....", data, sortType);
  ...
  const sortData = () => {
    ...
    if (parseInt(sortType) === Sort.LowToHigh) {
      const sortedArr = arr.sort(sortLowPrice);
      setFilteredArr(sortedArr);
    } else if (parseInt(sortType) === Sort.HighToLow) {
      const sortedArr = data.sort(sortHighPrice);
      setFilteredArr(sortedArr);
    } else if (parseInt(sortType) === Sort.Newest) {
      const sortedArr = data.sort(sortNewest);
      setFilteredArr(sortedArr);
    } else if (parseInt(sortType) === Sort.Oldest) {
      const sortedArr = data.sort(sortOldest);
      setFilteredArr(sortedArr);
    } else {
      setFilteredArr(data);
    }
