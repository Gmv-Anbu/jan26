...
const AuctionDetailsPage = ({ itemID, isPath, backBtnName, headerName, relatedProductsHeading }: DetailsProps) => {
  ...
  const getDetailsById = async (Id = itemID) => {
    ...
    .then(res => {
      if (res.status == 200) {
        const { id, name, refName, refNo, shortDescription, primarySalePrice, mainAssetUrl, description, currencyData, assetMediaFileData, isFavorite } = res.data.data
        ...
        let priceTrim: any = +primarySalePrice
        priceTrim = priceTrim.toFixed(2)
        const transformedName = name
        const transformedRefNo = refNo.replace('REF.', '')
        const transformedRefName = refName
        setCollectionDetails({
          id: id,
          title: transformedName,
          ref: transformedRefNo,
          brand: transformedRefName,
          price: `${currencyData.currencyCode} ${priceTrim}`,
        ...
        <Container>
          <ProductWrapper>
            ...
            <DetailsWrapper>
             ...
             <ArchiveDBox>
                <h4>Patek Philippe {collectionDetails.title}</h4>
                <p className='ref'>Ref. 3418A</p>
                <h2>“American” Calendar</h2>
                

                <div className='margin-bottom-2vh'>
                    <p>Estimate</p>
                    <h4 className='font-weight-600'>$10000-$25000</h4>
                </div>
                <div>
                    <p>Won Bid Amount</p>
                    <h4 className='font-weight-600'>$25000</h4>
                </div>
                <div className='margin-top-10vh'>
                    <p>{"Buyer's Premium"}</p>
                    <h5 className='font-weight-600 font-2rem'>$25000</h5>
                </div>
              </ArchiveDBox>
...
export default memo(AuctionDetailsPage)
