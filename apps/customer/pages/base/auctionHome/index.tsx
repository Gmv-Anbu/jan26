...
const Auction = () => {
  ...
  const [activeAuction, setActiveAuction] = useState('current');
  ...
  const getAuctions = () => {
    ...
    API.getAuctions(currentPage, 10, activeAuction, filter).then((res) => {
      setIsLoading(false);
  ...
  const calculateRemainingTimeForEachItem = (tempArray) => {
    if (tempArray.length > 0) {
      tempArray.forEach((obj) => {
        if (activeAuction == 'current') {
          obj['remainingTime'] = calculateRemainingTime(obj?.endTime)
        } else if (obj?.data && obj?.data?.length && obj?.data?.length > 0) {
          obj.data[0]['remainingTime'] = moment(obj?.data[0].startTime).format('DD-MMM-YYYY')
  ...
  return (
    <MyNftWrapper>
      <Container className='max-width-100'>
        ...
        <Banner heading={activeAuction === 'current' ? 'Live Auctions' : 'Upcoming Auctions'} description={''} height={'450px'} />
        <ListBox className='padding-10vw'>
          <EventTimeContainer>
            ...
            <EventTime onClick={() => handleTab('current')} className={activeAuction === 'current' ? 'active' : ''}>
              Current Auctions ({pageMeta?.currentTotal})
            </EventTime>
            <EventTime onClick={() => handleTab('upcoming')} className={activeAuction === 'upcoming' ? 'active' : ''}>
              Upcoming Auctions ({pageMeta?.upcomingTotal})
            </EventTime>
          </EventTimeContainer>
          <div className='pad-lef-80 display-flex margin-bottom-3vh'>
            ...
            <div className='filter-image'>
              ...
              <div className={displayFilter ? 'filter-options display-block' : 'filter-options'}>
                ...
                {/* <h4 className='flex-90'>Estimate</h4>
                <div className='display-flex margin-bottom-3vh'>
                  <div>
                    
                    <input type="text" onChange={(e) => { }} />
                  </div>
                  <div className='mar-lr-10'>
                ...
                {/* <div className='margin-bottom-3vh'>
          ...
          {auctionCards?.length ? (
            <ListItemsComp items={auctionCards} activeAuction={activeAuction} showArchiveModal={showArchiveModal} totalPage={Math.ceil(activeAuction === 'current' ? (pageMeta?.currentTotal / 10) : (pageMeta?.upcomingTotal / 10))} currentPage={pageMeta?.page} updatePage={updatePage} />
          ) : (
...
function ListItemsComp({ items, showArchiveModal, totalPage, currentPage, updatePage, activeAuction }) {
  return (
    <>
      {activeAuction == 'current' ? (
        <>
          <GridContianer>
            {items.map((product, i) => {
              if (window.screen.availWidth < 549) {
                if (i < 4) {
                  return <Card key={i} data={activeAuction == 'current' ? product : product?.data?.length ? product?.data[0] : []} showArchiveModal={showArchiveModal} activeAuction={activeAuction} />
                }
              } else {
                return <Card key={i} data={activeAuction == 'current' ? product : product?.data?.length ? product?.data[0] : []} showArchiveModal={showArchiveModal} activeAuction={activeAuction} />
              }
        ...
        {items.map((product, i) => {
          return (
            <>
              <div className='display-flex'>
                <h2 className='upcoming-title' key={i}>{product?.title}</h2>
                {product?.data?.length>4 && <p className="viewall">View All <ArrowForwardIosIcon className='viewall-arrow' sx={{ color: '#5C5C5C' }} fontSize='medium'></ArrowForwardIosIcon></p>}
              </div>
              <GridContianer>
                {product?.data?.map((upcomingProduct, i) => {
                  if (i < 4) {
                    if (window.screen.availWidth < 549) {
                      if (i < 4) {
                        return <Card key={i} data={upcomingProduct} showArchiveModal={showArchiveModal} activeAuction={activeAuction} />
                      }
                    } else {
                      return <Card key={i} data={upcomingProduct} showArchiveModal={showArchiveModal} activeAuction={activeAuction} />
                    }
...
function Card({ data, showArchiveModal, activeAuction }) {
  ...
  return (
    <CardWrapper>
      <p className='ends-font'><span className='ends-title'>{activeAuction == 'current' ? 'Ends In:' : ''} </span> <span className='time-card'>{data?.remainingTime ? data?.remainingTime : '-'}</span></p>
      ...
      <div className="btn-box">
        <div className="price-box">
          {' '}
          <p className='current-price-font'>{activeAuction == 'current' ? 'Current Price':'Estimate'}</p>
          <h4>$ {data?.bidAmount}</h4>
        </div>
        {activeAuction == 'current' ? <CardButton className='bid-now-width' onClick={() => router.push(`/base/auction/355`)}>Bid Now</CardButton> : <CardButton className='bid-now-width'>Follow</CardButton>}
      </div>
    </CardWrapper>
  )
}
